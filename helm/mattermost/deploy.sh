...
install () {
  ...
  sudo -u postgres bash -c "psql -U postgres -c 'CREATE DATABASE $DBNAME OWNER $USERNAME;'"  && \
  sudo -u postgres bash -c "psql -U postgres -c 'GRANT CONNECT ON $DBNAME TO $USERNAME; GRANT USAGE ON SCHEMA public TO $USERNAME; GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO $USERNAME;'"  && \
