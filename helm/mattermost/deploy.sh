...
install () {
  ...
  sudo -u postgres bash -c "psql -U postgres -c 'GRANT CONNECT ON $DB_NAME TO $USERNAME; GRANT USAGE ON SCHEMA public TO $USERNAME; GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO $USERNAME;'"  && \
